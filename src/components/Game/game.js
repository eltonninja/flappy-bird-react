...
export function initializeGame({
  ...
  function update() {
    ...
    else if (Phaser.Input.Keyboard.JustDown(upButton)) flapBird();
    else {
    ...
    pipesGroup.children.iterate(function (child) {
      ...
      else child.setVelocityX(-birdSpeed);
    });

    gapsGroup.children.iterate(function (child) {
      child.body.setVelocityX(-birdSpeed);
    });
  ...
  function makePipes(scene) {
    ...
    const pipeTopY = Phaser.Math.Between(-gapSize - 20, gapSize + 20);
